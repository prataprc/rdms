    ...
    H: Clone + BuildHasher + From<Vec<u8>> + Into<Vec<u8>>,
{
    fn len(&self) -> Result<usize> {
        match self.len() {
            Some(n) => Ok(n),
            None => err_at!(NotImplemented, msg: "Fuse8 does not implement length"),
        }
    }
    ...
    fn from_bytes(mut buf: &[u8]) -> Result<(Self, usize)> {
        ...
        Ok((err_at!(FailCbor, Fuse8::<H>::from_cbor(cbor_val))?, n))
    }

    fn or(&self, _other: &Self) -> Result<Self> {
        err_at!(NotImplemented, msg: "Fuse8 does not implement or() method")
    }
}
