    ...
    D: DiskIndexFactory<K, V>,
{
    ...
    levels: Mutex<Levels<K, V, M, D>>, // snapshots
...
where
    ...
    D: DiskIndexFactory<K, V>,
{
    fn new(m0: M::I) -> Levels<K, V, M, D> {
        Levels {
            m0: Snapshot::new_write(m0),
            m1: None,
    ...
    D::I: Footprint,
{
    ...
    pub fn new(
        ...
        let levels = Levels::<K, V, M, D>::new(mem_factory.new(name)?);
        Ok(Box::new(Dgm {
    ...
    //    disk_factory: D,
    //) -> Result<Box<Dgm<K, V, M, D>>> {
    //    let root = Self::open_root_file(dir, name);
    ...
    fn open_root_file(dir: &ffi::OsStr, name: &str) -> Result<Root> {
        use crate::error::Error::InvalidFile;
...
//    fn new(
//        name: &str,
//        rs: Arc<Mutex<Rs<K, V, M, D>>>, // pre-build reader
//    ) -> DgmReader<K, V, M, D> {
//        DgmReader {
//            name: name.to_string(),
//            rs,
...
//    }
//
//    fn get_readers(&self) -> Result<MutexGuard<Rs<K, V, M, D>>> {
